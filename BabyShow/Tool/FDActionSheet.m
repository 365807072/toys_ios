...
-(id)initWithTitle:(NSString *)title delegate:(id)delegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitlesArray:(NSArray *)otherButtonArray{
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        _title = title;
        _delegate = delegate;
        _cancelButtonTitle = cancelButtonTitle;
        ...
        _buttonTitleArray = [NSMutableArray array];
        va_list args;
       // va_start(args, otherButtonArray);
        if (otherButtonArray) {
            [_buttonTitleArray addObjectsFromArray:otherButtonArray];
            while (1) {
...
- (void)setTitleColor:(UIColor *)color fontSize:(CGFloat)size {
    if (color != nil) {
        _titleLabel.textColor = [BBSColor hexStringToColor:@"d7d7d7"];
    }
