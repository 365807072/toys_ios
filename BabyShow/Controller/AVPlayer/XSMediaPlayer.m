...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _backImg = [[UIImageView alloc]initWithFrame:frame];
        ...
        UITapGestureRecognizer *touchTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(pushDetailView)];
        ...
        [self.maskView.playbtn  addTarget:self action:@selector(startAction:) forControlEvents:UIControlEventTouchUpInside];
        [self.maskView.backBtn addTarget:self action:@selector(toSmallScreen) forControlEvents:UIControlEventTouchUpInside];
        [self.maskView.backTextBtn addTarget:self action:@selector(toSmallScreen) forControlEvents:UIControlEventTouchUpInside];
        [self.maskView.backBtnSmall addTarget:self action:@selector(toBack) forControlEvents:UIControlEventTouchUpInside];
        [self.maskView.shareBtn addTarget:self action:@selector(sendShareNot) forControlEvents:UIControlEventTouchUpInside];
...
-(void)toBack{
    ...
    [self.navFromSmall popViewControllerAnimated:YES];
}
-(void)setHorizontalOrVerticalScreen:(BOOL)isHV{
    if (isHV == NO) {
        //竖屏
...
//        babyShowPlayerVc.videoUrl = _videoUrlString;
//        babyShowPlayerVc.isHV = self.isHV;
//        babyShowPlayerVc.img_id =[NSString stringWithFormat:@"%@",_imgID];
//        [_nav pushViewController:babyShowPlayerVc animated:YES];
    }
    
}
-(void)settingBackgroupImg:(UIImage *)img{
    _backImg.image = img;
    _imgBack = [[UIImageView alloc]initWithFrame:_backImg.frame];
    _imgBack.image = [UIImage imageNamed:@"back_video_blurry"];
    [_backImg addSubview:_imgBack];
...
//    button.selected = !button.selected;
    self.maskView.startBtn.selected = !self.maskView.startBtn.selected;
    if (self.maskView.startBtn.selected) {
...
-(void)sendShareNot{
    [[NSNotificationCenter defaultCenter]postNotificationName:@"sendShareNotification" object:nil];
}
-(void)toSmallScreen{
    self.maskView.backBtn.hidden = YES;
    self.maskView.backTextBtn.hidden = YES;
    self.maskView.backBtnSmall.hidden = NO;
    self.maskView.shareBtn.hidden = NO;
    self.maskView.startBtn.frame = CGRectMake(7, 9, 10, 10);
    self.maskView.currentTimeLabel.frame = CGRectMake(self.maskView.startBtn.frame.origin.x+self.maskView.startBtn.frame.size.width+2, 5,45, 20);
    

    [self removeFromSuperview];
    self.transform = CGAffineTransformIdentity;
    self.transform = CGAffineTransformMakeRotation(2*M_PI);
    self.frame = CGRectMake(0, 0,self.smallFrame.size.width,self.smallFrame.size.height);
    _backImg.frame = self.frame;
    _imgBack.frame = self.frame;
    self.playerLayer.frame = CGRectMake(0, 0,self.smallFrame.size.width,self.smallFrame.size.height);
...
-(void)toFullScreenWithInterfaceOrientation:(UIInterfaceOrientation )interfaceOrientation{
    ...
    self.maskView.backTextBtn.hidden = NO;
    [self.maskView.backTextBtn setTitle:self.imgTitle forState:UIControlStateNormal];
    NSDictionary *attributes = @{NSFontAttributeName:[UIFont systemFontOfSize:17]};
    CGSize nickSize = [self.imgTitle sizeWithAttributes:attributes];
    self.maskView.backTextBtn.frame = CGRectMake(40, 20, nickSize.width, 20);
    //self.maskView.backTextBtn.backgroundColor = [UIColor redColor];
    self.maskView.backBtnSmall.hidden = YES;
    self.maskView.shareBtn.hidden = YES;
    self.maskView.startBtn.frame = CGRectMake(20, 9, 10, 10);
    self.maskView.currentTimeLabel.frame = CGRectMake(self.maskView.startBtn.frame.origin.x+self.maskView.startBtn.frame.size.width+2, 5,45, 20);
    ...
    _backImg.frame = CGRectMake(0, 0, SCREENHEIGHT, SCREENWIDTH);
    _imgBack.frame = _backImg.frame;
