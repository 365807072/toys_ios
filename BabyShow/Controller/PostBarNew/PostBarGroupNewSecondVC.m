...
@interface PostBarGroupNewSecondVC ()<RefreshControlDelegate,UITableViewDataSource,UITableViewDelegate>{
...
@implementation PostBarGroupNewSecondVC
...
-(void) setHeadImgView{
    ...
    CGRect editBtnFrame = CGRectMake(SCREENWIDTH-42, 20, 33, 33);
    ...
    CGRect focusFrame = CGRectMake(SCREENWIDTH-78,  SCREENWIDTH*0.55-20, 68, 15);
    ...
    self.editBigbtn.frame = CGRectMake(SCREENWIDTH-50, 20, 50, 40);
    ...
    self.allBtn.frame = CGRectMake(5, SCREENWIDTH*0.55+7, 66, 30);
    [self.allBtn setBackgroundImage:[UIImage imageNamed:@"post_bar_all"] forState:UIControlStateNormal];
    [self.allBtn.titleLabel setFont:[UIFont systemFontOfSize:15]];
    [self.allBtn setTitleColor:[BBSColor hexStringToColor:@"000000"] forState:UIControlStateNormal];
    [self.allBtn setTitle:@"全部" forState:UIControlStateNormal];
    self.allBtn.imageView.transform = CGAffineTransformRotate(self.allBtn.imageView.transform, M_PI);
    [self.allBtn setImage:[UIImage imageNamed:@"post_bar_all_arrow"] forState:UIControlStateNormal];
    self.allBtn.imageEdgeInsets = UIEdgeInsetsMake(4, 45, 4, 0);
    self.allBtn.titleEdgeInsets = UIEdgeInsetsMake(0, -7, 0, 15);
    [self.grayHeadView addSubview:self.allBtn];
    [self.allBtn addTarget:self action:@selector(changeIcon) forControlEvents:UIControlEventTouchUpInside];
    
    self.diaryBtn = [YLButton buttonWithFrame:CGRectMake(SCREENWIDTH-150,SCREENWIDTH*0.55+5, 70, 28) type:UIButtonTypeCustom backImage:[UIImage imageNamed:@"btn_group_diary"] target:self action:@selector(pushDiary) forControlEvents:UIControlEventTouchUpInside];
    self.diaryBtn.hidden = YES;
    [self.grayHeadView addSubview:self.diaryBtn];
    
    self.albumBtn = [YLButton buttonWithFrame:CGRectMake(SCREENWIDTH-150+75,SCREENWIDTH*0.55+5, 70, 28) type:UIButtonTypeCustom backImage:[UIImage imageNamed:@"btn_group_photo"] target:self action:@selector(pushAlbum) forControlEvents:UIControlEventTouchUpInside];
...
-(void)setClassTableView{
    ...
    if (_classTableView) {
        ...
        if (_classArray.count == 0) {
            if ([self.user_id intValue] == [LOGIN_USER_ID intValue]) {
                //有添加分类的按钮
                frame = CGRectMake(5, self.heightClassTableView, 115,76);
                
            }else{
                //直接显示全部
                frame = CGRectMake(5, self.heightClassTableView, 115,38);
                
            }
        }else{
            if ([self.user_id intValue] == [LOGIN_USER_ID intValue]) {
                //有添加分类的按钮
                frame = CGRectMake(5, self.heightClassTableView, 115,_classArray.count*38+38+38);
                
            }else{
                //直接显示全部
                frame = CGRectMake(5, self.heightClassTableView, 115,_classArray.count*38+38);
    ...
    _classTableView = [[UITableView alloc]initWithFrame:frame];
...
-(void)getClassListArray{
    ...
    NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:LOGIN_USER_ID,@"login_user_id",[NSString stringWithFormat:@"%ld",self.group_id],@"group_id", nil];
...
-(void)getHeaderViewData{
    ...
    [[HTTPClient sharedClient]getNewV1:kgroupHeadInfo params:params success:^(NSDictionary *result) {
        if ([[result objectForKey:kBBSSuccess]boolValue] == YES) {
            ...
            //是否展示相册
            NSString *is_album = infoDic[@"is_album"];
            if ([is_album isEqualToString:@"1"]) {
            ...
            //是否展示成长记录
            NSString *is_diary = infoDic[@"is_diary"];
            if ([is_diary isEqualToString:@"0"]) {
                self.diaryBtn.hidden = YES;
            }else{
...
-(void)getNewTableViewData{
    ...
    UrlMaker *urlMark = [[UrlMaker alloc]initWithNewV1UrlStr:@"getGroupListingV1" Method:NetMethodGet andParam:params];
    ASIHTTPRequest *request = [ASIHTTPRequest requestWithURL:urlMark.url];
    [request setDownloadCache:theAppDelegate.myCache];
    [request setCachePolicy:ASIUseDefaultCachePolicy | ASIFallbackToCacheIfLoadFailsCachePolicy];
    [request setCacheStoragePolicy:ASICachePermanentlyCacheStoragePolicy];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tableView isEqual:_classTableView]) {
        ...
        UILabel *nicknameLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 6, 115, 25)];
        nicknameLabel.backgroundColor = [UIColor clearColor];
        nicknameLabel.textAlignment = NSTextAlignmentCenter;
        nicknameLabel.textColor = [UIColor blackColor];
        nicknameLabel.font = [UIFont systemFontOfSize:14];
        [cell.contentView addSubview:nicknameLabel];
        UIImageView *lineView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 37, 115, 1)];
        lineView.backgroundColor = [BBSColor hexStringToColor:@"e8e8e8"];
        [cell.contentView addSubview:lineView];
        
        UIImageView *lineViewLeft = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 1, 38)];
        lineViewLeft.backgroundColor = [BBSColor hexStringToColor:@"e8e8e8"];
        [cell.contentView addSubview:lineViewLeft];
        UIImageView *lineViewRight = [[UIImageView alloc]initWithFrame:CGRectMake(114, 0, 1, 38)];
        lineViewRight.backgroundColor = [BBSColor hexStringToColor:@"e8e8e8"];
