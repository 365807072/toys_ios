//
//  WorthBuyMakeAShowViewController.m
//  BabyShow
//
...
#import "WorthBuyMakeAShowViewController.h"
#import "SDWebImageManager.h"
#import "AlbumListViewController.h"
#import "CHTumblrMenuView.h"

@interface WorthBuyMakeAShowViewController ()

@property (nonatomic, strong) UIScrollView *scrollview;
@property (nonatomic, strong) UITextView *textView;
@property (nonatomic, strong) WorthBuyToolBarView *toolBarView;
...
float basicy1=35;
float imageviewWidth1=75;
float imageviewSeperate1=4;
float toolBarHeight1=40;

@implementation WorthBuyMakeAShowViewController
...
-(void)setToolBarView{
    ...
    self.toolBarView=[[WorthBuyToolBarView alloc]init];
...
-(void)setPhotoView{
    ...
    CGRect photoAreaFrame=CGRectMake(0, basicy1, SCREENWIDTH, SCREENHEIGHT-basicy1-toolBarHeight1);
...
-(void)setScrollview{
    ...
    self.scrollview=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT-toolBarHeight1)];
    self.scrollview.contentSize=CGSizeMake(SCREENWIDTH, SCREENHEIGHT-toolBarHeight1);
...
-(void)textViewDidChange:(UITextView *)textView{
    ...
    if (height>30) {
        ...
        self.photoAreaView.frame=CGRectMake(0, basicy1, SCREENWIDTH, SCREENHEIGHT-basicy1-toolBarHeight1);
        self.textView.frame=newframe;
    }
    
    if (textView.text.length>=65) {
        textView.text=[textView.text substringWithRange:NSMakeRange(0, 65)];
    }
...
-(void)send{
    ...
    [menu addMenuItemWithTitle:@"发布" andIcon:[UIImage imageNamed:@"btn_postbar_postbar_only"] andSelectedBlock:^{
    ...
    [menu addMenuItemWithTitle:@"同时发布到秀秀" andIcon:[UIImage imageNamed:@"btn_postbar_myshow_also"] andSelectedBlock:^{
        ...
        [LoadingView startOnTheViewController:self];
        dispatch_async(dispatch_queue_create(DISPATCH_QUEUE_PRIORITY_DEFAULT, NULL), ^{
            if (self.type==WORTHBUYADDATOPICDEFAULT) {
                [self netWithType:@"2"];
            }else if (self.type==WORTHBUYADDATOPICWORTHBUY){
                [self netWithType:@"3"];
            }
...
-(void)netWithType:(NSString *) type{
    ...
    if (description.length>65) {
        ...
        dispatch_async(queue, ^{
            ...
            [BBSAlert showAlertWithContent:@"描述最多只能65字" andDelegate:self];
        ...
        if ((imgageArray.count || imgStrArray.count) && description.length ) {
            ...
            if (self.type==WORTHBUYADDATOPICDEFAULT) {
                ...
                if (self.buy_class) {
                    [param setObject:self.buy_class forKey:@"buy_class"];
                }else{
                    dispatch_queue_t queue=dispatch_get_main_queue();
                    dispatch_async(queue, ^{
                        [LoadingView stopOnTheViewController:self];
                ...
                [netAccess getDataWithStyle:NetStylePostBarAddTopicWorthBuy andParam:param];
            }
            ...
            dispatch_async(queue, ^{
                [LoadingView stopOnTheViewController:self];
                [BBSAlert showAlertWithContent:@"描述和图片不能为空哦" andDelegate:self];
                self.sendBtn.enabled=YES;
...
-(void)makeImageViewsWithPhotosArray:(NSArray *) PhotoArray{
    ...
    for (int i=0; i<PhotoArray.count; i++) {
        ...
        float blockWidth=imageviewWidth1+imageviewSeperate1;
        int j=i%4;
        
        if (i<=3) {
            
            imageviewFrame=CGRectMake(blockWidth*j+imageviewSeperate1, imageviewSeperate1+10, imageviewWidth1, imageviewWidth1);
            
        }else if (i<=5){
            
            imageviewFrame=CGRectMake(blockWidth*j+imageviewSeperate1, blockWidth+imageviewSeperate1+10, imageviewWidth1, imageviewWidth1);
...
-(void)succeed{
    ...
    delegate.worthbuyHasNewTopic=YES;
    if ([self.delegate respondsToSelector:@selector(changeRefreshChannel:)]) {
        [self.delegate changeRefreshChannel:self.buy_class];
    }
