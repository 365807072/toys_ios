...
-(NSString*) createMd5Sign:(NSMutableDictionary*)dict
{
    ...
    //输出Debug Info
    [debugInfo appendFormat:@"是这个么  MD5签名字符串：\n%@\n\n",contentString];
...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)temp_name:(NSString *)temp_name tempno:(NSString *)tempno temp_price:(NSString *)temp_price
{
    //订单标题，展示给用户
    //NSString *order_name    = [temp_name stringByAppendingString:tempno];
    NSString *order_name    = temp_name;
    //订单金额,单位（分）
    
    NSInteger price = [temp_price doubleValue]*10*10;
    NSString *finalPrice = [NSString stringWithFormat:@"%ld",(long)price];
    //微信支付
    NSString *order_price   = finalPrice;//1分钱测试
    NSLog(@"price = %@,%ld,%@,%@",temp_price,price,finalPrice,order_price);
    //预付单参数订单设置
    //================================
    srand( (unsigned)time(0) );
    NSString *noncestr  = [NSString stringWithFormat:@"%d", rand()];
    NSString *orderno   = [NSString stringWithFormat:@"%@",tempno];
