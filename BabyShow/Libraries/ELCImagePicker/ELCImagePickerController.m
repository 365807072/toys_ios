...
//        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"Only %d photos please!", nil), self.maximumImagesCount];
        NSString *title = [NSString stringWithFormat:@"数量超出限制"];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"每次只可以上传%d张相片哦", nil), self.maximumImagesCount];
...
- (void)selectedAssets:(NSArray *)assets
{
	...
	for(ALAsset *asset in assets) {
        ...
        if (date) {
            [workingDictionary setObject:date forKey:ALAssetPropertyDate];
        }
